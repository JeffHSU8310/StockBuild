...
# 【1.7 官方指南實證】舊寫法 `Indexs.TSE["001"]` → 新寫法 `Indexs.TSE["IX0001"]`。
# 也就是新代碼 = 'IX' + 舊代碼補零到 4 位。加權舊代碼 001 → IX0001 是文件明載的;
# 櫃買**文件沒有列出**,依同一規則由舊代碼 101 推得 IX0101 (推論,非證實)。
# 因為是推論,IX0101 與 IX0002 兩種可能都放進候選,再退回舊代碼——猜錯也不會壞,
# 只是多試幾個。實機確認後可以把沒中的那個拿掉。
INDEX_CANDIDATES = {
    'TSE': ('IX0001', 'TSE001', '001'),                        # 加權指數 (IX0001 文件明載)
    'OTC': ('IX0101', 'IX0002', 'OTC101', 'OTC001', '101'),    # 櫃買指數 (IX0101 為推論)
}
...
INDEX_SYMBOLS = frozenset({
    ...
    'IX0001', 'IX0101', 'IX0002',        # 1.7 (IX0101/IX0002:櫃買的兩種可能)
})
