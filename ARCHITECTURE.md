...
- `strategy_engine.py` 的條件庫（ADR-057 起）新增一組**當日（盤中）漲跌幅**
  ...
  期貨的「當日」照 ADR-007 的交易日規則（15:00 後歸下一個交易日），
  規則與 `futures_session.resample_future_session()` 共用同一份
  （`session_date_of()` / `has_night_session_bars()`）。
...
**一個設定只能管一件事;能用市場的自然邊界就不要加旋鈕**（ADR-155，PITFALLS
P-137）。定時下單原本有一個「有效窗口(分)」,ADR-147 加它是為了**取價的寬限期**
(那一秒沒抓到報價怎麼辦);ADR-154 要限制「等條件等多久」時沿用了同一個數字,
於是一個旋鈕管兩件不相干的事 —— 使用者的心智模型只會對到其中一件。
正確答案是**今天收盤**:那是市場定的邊界,不是使用者要調的參數。
`strategy_engine.timed_deadline_minute_of()` 依交易種類回傳收盤分鐘數,常數一律
取自 `core/market_session`(期貨含夜盤跨午夜,而定時下單的「今天」是日曆日,
所以那一種誠實地以自然日結束為界並寫明)。移除設定時連策略檔的 key 一起 `pop`
掉,並用診斷的原始碼層級斷言守著它不復活。
連帶的教訓:把一個上限從 5 分鐘放寬到整個交易日時,要**回頭看所有靠它撐住的
東西** —— `_qt_timed_log_once` 拿整句訊息去重,而訊息含每 2 秒都在變的現價,
5 分鐘時上限 150 行看不太出來,3 小時就是幾千行。去重要用穩定的**原因鍵**。

**共用了純函式不等於共用了規則 —— 流程控制也要收斂**（ADR-154，PITFALLS
P-136）。定時下單的 `should_fire_timed` / `check_timed_entry` / `mark_timed_done`
三個純函式實盤與回測共用,但「**什麼時候可以收尾**」這個決定原本留在各自的
呼叫端:實盤是「條件不成立 → 重試」,回測是「不管成不成立都收尾」。
同一份策略兩邊績效對不上,而分岔藏在一個 `if` 的位置,程式碼看起來已經收斂了。
規則本身:`mark_timed_done()` 只有「**真的送出去**」與「**窗口過了**」兩種情況
可以呼叫,「條件不成立」不算處理完 —— 指定時刻是「從這裡開始看」不是
「只看這一眼」(使用者原話:「在設定的時間之後有符合策略條件,還是要下單」)。
規則寫在純函式的 docstring(單一出處),並由診斷的**原始碼層級**斷言把兩條路
釘在一起 —— 行為斷言只驗得到自己那一條。訊息也必須跟行為一致:實盤原本行為
是對的,卻寫著「今天不送單」,那一樣是 bug。

**掛單價與成交價是兩個數字,記帳只能用後者**（ADR-153，PITFALLS P-134）。
限價單「讓價 N 檔」設的是**願意接受的價格上限**,不是預期成交價 —— 對手簿上
第一檔量夠就全部成交在第一檔。原本的實作把掛價直接當成交價寫進 `apply_fill()`,
而樂觀成交模型(ADR-035)下那個價會變成後續所有損益/停損停利的基準,誤差一路
複製下去。單一出處是 `quote_book.walk_fill()`:沿對手簿逐檔吃量,回傳
`avg_price` / `filled_qty` / `fully_filled` / `levels`,`describe_fill()` 負責
把它講成人話。**送給券商的是掛價,記進帳的是預估均價**,兩個數字都要出現在
訊息裡;實單還要標明「實際成交價以券商回報為準」—— 我們算的是估計值,
真值只有券商回報有。

**定時類觸發問的是「此刻符不符合」,不是「今天曾經符合過」**（ADR-153，
PITFALLS P-135）。使用者設 12:01 下單,語意是「12:01 那一刻拿 A 的當下價去看
條件」。原本走的是最後一根**已收盤 K 棒**,那可能是好幾分鐘前、甚至早盤的
數字,結果是「早上跌 0.5% 觸發過,中午已經漲回來了照樣買」。
`strategy_engine.check_timed_entry(..., live_price=)` 對
`LIVE_ENTRY_CONDITIONS`(當日漲跌幅那類)**強制改用 `use='收盤價'` 配當下價**
求值,其餘條件仍走 `eval_conditions`。當下價要送得到,GUI 層得先讓定時下單的
A 標的進 `snapshots()` 監看名單(`_qt_timed_a_sym` → `runtime['_live_a_price']`),
新能力只接一條路就是 P-131 那個坑。**條件不成立要出聲**:記一則「到達指定時刻
但當下條件不成立,今天不送單」,安靜跳過會讓使用者以為系統沒在跑。

**一天一則的通知,最糟的失敗模式是安靜漏掉**（ADR-132/152）。兩個做法要一起用:
「**過了時刻就送**」而不是「剛好等於那一分鐘」(程式那分鐘沒開就整天不發),
再配上**每日去重**讓它不會變成重複轟炸;而且**資料還沒好時不可以標記成已送**,
否則那天就永遠不會補送。這類通知還要列進 `telegram_notify.ALWAYS_SEND_TAGS`,
免得被 ADR-148 的 burst 節流合併掉。終極波段的每日晨間狀態報告
(`chukuangren_band.daily_status_report()`)就是照這個模式做的,算式一律沿用
`on_daily_close()` 那一份 —— 報告與引擎各算一份遲早分歧(P-67)。

**每加一個「什麼時候可以下單」的限制,就回頭問回測知不知道**（ADR-151，
PITFALLS P-133）。`timed_entry` 與 `market_session` 原本在 `core/backtest.py` 裡
一次都沒出現 —— 使用者設 10:30 卻在 09:03 進場,而零股 09:10 才開盤。
定時下單啟用時是**唯一**的進場觸發(抑制寫在 `filter_intents_by_time()`,
因為 Buy & Hold 會提早 return);「這份資料有沒有盤中時刻」看**實際 K 棒間距**
不看宣告的 timeframe;零股沒有 K 線,回測用的是整股 K 棒,報告必須講明。

**回測必須照策略的風控設定跑**（ADR-150，PITFALLS P-132）。回測**從來沒有
呼叫過 `risk_check`**,而且還把 `max_trades_per_day` 放寬成 `10**9` ——
使用者設「每日進場上限 1」,回測卻一天進場 397 次。每日進場上限**不是防手殘,
是策略規則**,忽略它等於拿一份過度樂觀的數字在做決策。
`run_backtest(apply_risk_guards=True)` 是預設值,要看純訊號績效才關掉,
而且報告的總結列會明講這次有沒有套用。

兩個實作陷阱:回測迴圈的 `now_ts` 是**K 棒序號不是秒**,風控要另外算真實時間戳
(`ts.timestamp()`),而且 `risk_check` 與 `apply_fill` 必須用**同一個時鐘**;
換日重置的單一出處是 `strategy_engine.reset_daily_counters()`,回測的自訂策略
路徑不經過 `evaluate_strategy()`,得自己呼叫同一份。

**新增「取價/風控」層級的能力時,要把既有的每一條路徑都接上**（ADR-149，
PITFALLS P-131）。ADR-147 把五檔取價接上了「定時下單」那條新路,但零股走
一般 K 棒進場、即時進場、即時停損時掛價仍是整股價 —— `_qt_get_book()` 全檔
只被呼叫一處就是證據。取價規則的單一出處是 `core/quote_book.py`,
**不可以為同一件事開第二個模組**(診斷直接斷言那個檔名不存在)。
零股常駐訂閱的範圍是「定時下單 ∪ 零股策略」(`_qt_book_targets()`)。
**進場與出場的失敗處置刻意相反**:進場取不到五檔就**不送單**(錯價單會因為
樂觀成交模型變成幽靈部位),出場取不到就**退回整股價照樣送**(出場走不掉是
部位裸奔)。出場的觸發價與掛單價都用**對手價** —— 停損問的是「現在出得掉
什麼價」。進場時間閘門的單一出處是 `strategy_engine.entry_time_gate()`,
K 棒路徑與即時進場通道共用(ADR-145 那條原本一行時間判斷都沒有)。
