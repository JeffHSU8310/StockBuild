...
5. **對 `snapshots()` 的呼叫必須節流，不可以在迴圈裡無限制猛打。**
   ...
   當天報價全部失效。目前定案 (2026-07-24 ADR-094 調整)：無串流 fallback
   快照間隔 ≥ 3 秒 (查證 shioaji 官方文件 snapshots/ticks/kbars 等查詢合計
   10 秒 50 次上限，3 秒一次仍有約 5 倍安全margin，詳見 DECISIONS_ADR094.md)。
   若要再調整這個數字，一樣必須先查 shioaji 官方文件的流量限制並記錄依據到
   DECISIONS.md。
