...
class StockTradingAppPro(tk.Tk):
    ...
    def create_widgets(self):
        ...
        qt_bar = tk.Frame(self.quant_tab_frame, bg="#1A2026")
        qt_bar.pack(side=tk.TOP, fill=tk.X, padx=4, pady=(2, 0))
        self.lbl_qt_status = tk.Label(qt_bar, text="🔴 自動交易未啟動 (安全)", bg="#1A2026",
                                       fg="#FF5252", font=('微軟正黑體', 10, 'bold'))
        self.lbl_qt_status.pack(side=tk.LEFT, padx=(2, 10))
        self.btn_qt_arm = tk.Button(qt_bar, text="🟢 啟動自動交易 (需確認)", bg="#00C853", fg="black",
                                     relief="flat", font=('微軟正黑體', 9, 'bold'), padx=10, pady=2,
                                     command=self._qt_open_arm_dialog)
        self.btn_qt_arm.pack(side=tk.LEFT, padx=2)
        tk.Button(qt_bar, text="⛔ 全部停止", bg="#FF1744", fg="white",
                  relief="flat", font=('微軟正黑體', 9, 'bold'), padx=12, pady=2,
                  command=self._qt_stop_all).pack(side=tk.LEFT, padx=6)
        tk.Button(qt_bar, text="💰 模擬帳戶", bg="#FFB300", fg="black",
                  relief="flat", font=('微軟正黑體', 9, 'bold'), padx=10, pady=2,
                  command=self._qt_open_paper_window).pack(side=tk.LEFT, padx=6)
        tk.Label(qt_bar, text="每次開啟程式總開關都是關閉;新策略預設「模擬」→ 成交記入模擬帳戶",
                 bg="#1A2026", fg="#8A99AD", font=('微軟正黑體', 9)).pack(side=tk.LEFT, padx=8)
        qt_cols = ("name", "symbol", "tf", "direction", "conds", "mode", "status", "today", "pos")
        qt_headings = {"name": "策略名稱", "symbol": "商品", "tf": "週期", "direction": "方向",
                        "conds": "進場條件", "mode": "模式", "status": "狀態", "today": "今日次數", "pos": "持倉"}
        self.tree_quant = ttk.Treeview(self.quant_tab_frame, columns=qt_cols, show="headings", height=4, style='Trades.Treeview')
        widths = {"name": 110, "symbol": 70, "tf": 55, "direction": 50, "conds": 240,
                  "mode": 50, "status": 70, "today": 65, "pos": 90}
        for c in qt_cols:
            self.tree_quant.heading(c, text=qt_headings[c])
            self.tree_quant.column(c, width=widths[c], anchor="center")
        self.tree_quant.tag_configure('qt_on', foreground='#FF1744', background='#12161A')     # 實單紅
        self.tree_quant.tag_configure('qt_sim', foreground='#29B6F6', background='#12161A')    # 模擬藍
        self.tree_quant.tag_configure('qt_off', foreground='#8A99AD', background='#12161A')    # 停用灰
        # 【第十七輪修正 第4項】按鈕列改「先 pack + side=BOTTOM」:原本 Treeview
        # 先 pack 且 expand=True,分頁高度不足時後 pack 的按鈕列會被整條擠出可視
        # 範圍 (P-44),使用者就「看不到新增策略」。按鈕列優先保留空間、永遠可見,
        # 策略清單改為可被壓縮的那個。
        qt_btns = tk.Frame(self.quant_tab_frame, bg="#1A2026")
        qt_btns.pack(side=tk.BOTTOM, fill=tk.X, padx=4, pady=(2, 3))
        for txt, bg, cmd in (("➕ 新增策略", "#29B6F6", self._qt_new_strategy),
            ...
            fg = "white" if bg in ("#5A6472", "#AB47BC") else "black"
            tk.Button(qt_btns, text=txt, bg=bg, fg=fg, relief="flat",
                      font=('微軟正黑體', 9, 'bold'), padx=10, pady=3, command=cmd).pack(side=tk.LEFT, padx=2)
        self.tree_quant.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=4, pady=2)
    ...
    SJ_DAYS = {"1分K": 5, "5分K": 30, "15分K": 60, "30分K": 60, "60分K": 90,
               "日K": 365, "周K": 1095, "月K": 1825}
    QUICK_DAYS = {"日K": 45, "周K": 240, "月K": 600}   # 兩段式第一段的小範圍
    ...
    def _resample_sj_df(self, sj_df, tf, asset_type=None):
        ...
        if tf in ["日K", "周K", "月K"]:
            if asset_type == "future":
                ...
                return self._resample_future_session(sj_df, tf, agg_dict)
    ...
    def _looks_like_futures_symbol(sym):
        """
        【第十一輪修正】判斷代號「長得像期貨完整代號」:3 個英文字母 + (R1/R2 或
        6 位月份數字),如 TXFR2、TXF202609、CDF202607。
        修正前的市場自動判斷用「含數字=台股」——期貨完整代號本來就含數字
        (TXFR2 的 2!),被誤判成台股 → 合約查無、自選股報價全 '--'。
        期貨樣式判斷必須排在「含數字」判斷之前。
        """
        s = str(sym or '').upper()
        if len(s) < 5 or not s[:3].isalpha():
            return False
        tail = s[3:]
        return tail in ('R1', 'R2') or (len(tail) == 6 and tail.isdigit())
        ...
        try:
            ...
            if is_taiwan_instrument:
                if adjust_flag and self.asset_type == "stock":
                    # 【ADR-011】還原權息僅 yfinance 支援;台股用 shioaji 此設定暫不生效,明確告知。
                ...
                if cached_raw is not None and not cached_raw.empty:
                    try:
                        ...
                        if _publish(df_cached, full_ui=True):
                            ...
                            cached_len = len(df_cached.dropna(subset=['Open', 'High', 'Low', 'Close']))
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        if quick_raw is not None and not quick_raw.empty:
                            ...
                            if _publish(df_quick, full_ui=True):
                                quick_len = len(df_quick.dropna(subset=['Open', 'High', 'Low', 'Close']))
                    except Exception as e:
                        if self._looks_like_session_dead(e):
                            self._mark_session_dead()
                ...
                try:
                    # 【第二十輪修正】完整段下載失敗時自動縮短範圍重試:
                    # 期貨「近月合約」上市往往才 1~2 個月,一次要 365 天歷史,
                    # 券商端可能直接回錯 (使用者實例:小型臺指近月同秒失敗)。
                    # 365天失敗 → 退 180 天 → 再退 90 天;每次失敗都把例外
                    # 型別+訊息記進日誌 (根因要證據,不再吞掉)。
                    raw = None
                    last_err = None
                    for try_days in (None, 180, 90):
                        try_start = start_dt if try_days is None else max(start_dt, end_dt - timedelta(days=try_days))
                        if try_days is not None and try_start >= end_dt:
                            continue
                        try:
                            raw = self._download_kbars_raw(contract, try_start, end_dt)
                            if try_days is not None:
                                self.safe_after(0, self.log_message,
                                                f"【背景補全】完整範圍失敗,已改抓近 {try_days} 天成功 (該合約可能上市未滿一年,更早的資料不存在)。")
                            break
                        except Exception as e2:
                            last_err = e2
                            if self._looks_like_session_dead(e2):
                                raise
                    if raw is None:
        ...
        try:
            ...
            if rows:
                self.lbl_positions_summary.config(
                    text=f"共 {len(rows)} 檔 | 總未實現損益: {total_pnl:+,.0f} | 更新: {ts}",
            ...
            self.log_message(f"【庫存】已更新: {len(rows)} 檔,總未實現損益 {total_pnl:+,.0f}。")
        except Exception as e:
    ...
    def _open_positions_detail_window(self):
        ...
        tk.Label(bar, text=f"總未實現損益: {total_pnl:+,.0f}", bg="#1A2026",
    ...
    # ================= 【ADR-035】量化自動交易 =================
    QT_STRATEGY_FILE = "quant_strategies.json"
        ...
        try:
            ...
            for s in self.strategies:
                ...
                prepared.append((s['id'], (s.get('name',''), sym_disp, s.get('timeframe',''),
                                            s.get('direction',''), conds, s.get('mode','模擬'), status,
                                            rt.get('trades_today', 0), pos), tag))
            for iid in self.tree_quant.get_children():
                self.tree_quant.delete(iid)
            for iid, vals, tag in prepared:
                self.tree_quant.insert("", tk.END, iid=iid, values=vals, tags=(tag,))
        except Exception:
            pass

    def _qt_selected(self):
        try:
            iid = self.tree_quant.focus() or ((self.tree_quant.selection() or [None])[0])
            return next((s for s in self.strategies if s['id'] == iid), None)
        except Exception:
            return None
    ...
    def _qt_update_status_label(self):
        try:
            if not self._qt_running:
                self.lbl_qt_status.config(text="🔴 自動交易未啟動 (安全)", fg="#FF5252")
                self.btn_qt_arm.config(state=tk.NORMAL)
            else:
                live = any(s.get('enabled') and s.get('mode') == '實單' for s in self.strategies)
                if live:
                    self.lbl_qt_status.config(text="🔥 自動交易運轉中 — 含實單策略!", fg="#FF1744")
                else:
                    self.lbl_qt_status.config(text="🟢 自動交易運轉中 (全部模擬)", fg="#00E676")
                self.btn_qt_arm.config(state=tk.DISABLED)
        except Exception:
            pass
    ...
    def _qt_open_arm_dialog(self):
        ...
        for s in enabled:
            box.insert(tk.END, f"[{s.get('mode')}] {s.get('name')} — {s.get('symbol')} {s.get('timeframe')} {s.get('direction')} x{s.get('qty')}")
        if live:
    ...
    def _resolve_strategy_symbol_name(self, trade_type, symbol):
        ...
        try:
            if trade_type == '期貨':
                c = self._resolve_futures_contract(sym)
                if c is not None:
                    return (getattr(c, 'name', '') or getattr(c, 'symbol', sym)), True
                return None, False
            c = self.sj_api.Contracts.Stocks.get(sym)
            if c is not None:
        ...
        for s in list(self.strategies):
            ...
            try:
                ...
                if s.get('kind') == 'custom':
                    ...
                    intent = custom_strategy.decision_to_intent(decision, s, rt, float(df['Close'].iloc[-1]))
                ...
                for intent in intents:
                    ...
                    if s.get('mode') == '實單' and self._qt_running:
                        ...
                        try:
                            ...
                            self._qt_save_paper()
                            pnl_txt = f",此筆已實現 {rec['pnl']:+,.0f}" if intent['kind'] == 'CLOSE' else ""
    ...
    def _qt_open_custom_editor(self, strategy):
        ...
        dlg.configure(bg="#1A2026")
        self.center_window(dlg, 760, 660)
        dlg.transient(self)
        try:
            dlg.lift(); dlg.focus_force()
        except Exception:
        ...
        _lbl(top, "方向").grid(row=1, column=2, sticky='w', padx=(8, 0), pady=(6, 0))
        ...
        _lbl(top, "數量").grid(row=1, column=4, sticky='w', padx=(8, 0), pady=(6, 0))
        e_qty = _ent(top, s.get('qty', 1), 6); e_qty.grid(row=1, column=5, padx=4, pady=(6, 0))
        _lbl(top, "停損%").grid(row=2, column=0, sticky='w', pady=(6, 0))
        e_sl = _ent(top, s.get('stop_loss_pct', 0.0), 6); e_sl.grid(row=2, column=1, padx=4, pady=(6, 0))
        _lbl(top, "停利%").grid(row=2, column=2, sticky='w', padx=(8, 0), pady=(6, 0))
        e_tp = _ent(top, s.get('take_profit_pct', 0.0), 6); e_tp.grid(row=2, column=3, padx=4, pady=(6, 0))
        _lbl(top, "模式").grid(row=2, column=4, sticky='w', padx=(8, 0), pady=(6, 0))
        cb_mode = ttk.Combobox(top, values=['模擬', '實單'], width=7, state='readonly', style="BlackText.TCombobox")
        cb_mode.set(s.get('mode', '模擬')); cb_mode.grid(row=2, column=5, padx=4, pady=(6, 0))
        ...
        e_params = _ent(top, params_str, 40); e_params.grid(row=3, column=2, columnspan=4, sticky='w', padx=4, pady=(6, 0))

        tk.Label(dlg, text="on_bar(ctx) 程式碼:", bg="#1A2026", fg="#FFCA28",
                 font=('微軟正黑體', 9, 'bold')).pack(anchor='w', padx=12, pady=(6, 0))
        code_frame = tk.Frame(dlg, bg="#1A2026"); code_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(2, 4))
        ...
        def _validate_custom(strat):
            ...
            if strat['direction'] == '做空' and strategy_engine.trade_type_of(strat) != '期貨':
                return False, "做空僅支援期貨"
            # 語法檢查:能編譯 + 有 on_bar
            try:
                ns = {}
                exec(compile(strat['source_code'], '<check>', 'exec'), ns)
                if not callable(ns.get('on_bar')):
                    return False, "程式碼必須定義 on_bar(ctx) 函式"
            except Exception as e:
                return False, f"程式碼語法錯誤: {type(e).__name__}: {e}"
            return True, ""
        ...
        def _test_run():
            ...
            threading.Thread(target=self._qt_custom_test_worker, args=(copy.deepcopy(strat),), daemon=True).start()
        ...
        def _save():
            ...
            self._qt_save(); self._qt_save_state(); self._qt_refresh_tree()
            self.log_message(f"【自訂策略】「{strat['name']}」已儲存 ({strat['mode']})。")
            dlg.destroy()
    ...
    def _qt_custom_test_worker(self, strat):
        try:
            ...
            self.safe_after(0, self.log_message,
                            f"【自訂策略-試跑】✅ 執行成功!目前 (FLAT 狀態) 會回傳決策: {decision} "
    ...
    def _qt_open_editor(self, strategy):
        ...
        dlg.configure(bg="#1A2026")
        self.center_window(dlg, 720, 560)
        dlg.transient(self)
        try:
            dlg.lift(); dlg.focus_force()
        except Exception:
        ...
        lbl_name.grid(row=5, column=0, columnspan=4, sticky='w', pady=(2, 0))
        ...
        def _rebuild_params(event=None):
            ...
            for j, (k, lab, dv) in enumerate(spec):
                _lbl(param_frame, lab).grid(row=0, column=j*2, sticky='w', padx=(0, 2))
                e = _ent(param_frame, dv, 7); e.grid(row=0, column=j*2+1, padx=(0, 10))
                param_entries[k] = e
        ...
        _lbl(lists, "進場條件 (全部成立才進場)").grid(row=0, column=0, sticky='w')
        ...
        _lbl(lists, "出場訊號 (任一成立即出場)").grid(row=0, column=1, sticky='w')
    ...
    def _qt_backtest_selected(self):
        ...
        if getattr(self, '_backtest_running', False):
            self.log_message("【回測】已有回測進行中,請稍候。")
            return
    ...
    def _qt_backtest_ask_range(self, s):
        ...
        dlg.title("回測期間設定")
        dlg.configure(bg="#1A2026")
        self.center_window(dlg, 420, 220)
        dlg.transient(self)
        try:
            dlg.lift(); dlg.focus_force(); dlg.grab_set()
        except Exception:
        ...
        tk.Label(dlg, text="格式 YYYY-MM-DD;範圍越大下載越久。分K資料券商通常只保留近期。",
                 bg="#1A2026", fg="#8A99AD", font=('微軟正黑體', 8)).pack(pady=(6, 0))
        ...
        def _go():
            ...
            self.log_message(f"【回測】開始回測「{s.get('name')}」{e_start.get().strip()} ~ {e_end.get().strip()},下載中...")
            threading.Thread(target=self._qt_backtest_worker,
                             args=(copy.deepcopy(s), sd_, ed_), daemon=True).start()
    ...
    def _qt_backtest_worker(self, s, start_dt=None, end_dt=None):
        try:
            contract, asset_type = self._qt_resolve(s)
            if contract is None:
            ...
            raw = self._download_kbars_raw(contract, start_dt, end_dt)
            if raw is None or raw.empty:
                self.safe_after(0, self.log_message, "【回測】取不到歷史K線資料 (可能是新商品或休市)。")
                return
            df = self._resample_sj_df(raw, tf, asset_type=asset_type)
            # 依使用者指定範圍精確裁切 (下載可能多給)
            try:
            ...
            result = backtest.run_backtest(s, df, fee_rate=0.0, slippage_ticks=slip, tick_size=tick)
            self.safe_after(0, self._qt_show_backtest_report, s, df, result)
            m = result['metrics']
            self.safe_after(0, self.log_message,
                            f"【回測】「{s.get('name')}」完成:{m['trades']} 筆交易,總損益 {m['total_pnl']:+.2f},"
                            f"勝率 {m['win_rate']:.1f}%,最大回撤 {m['max_drawdown']:.2f}。")
        except Exception as e:
            self.safe_after(0, self.log_message, f"【回測】執行異常: {type(e).__name__}: {e}")
        finally:
    ...
    def _qt_show_backtest_report(self, s, df, result):
        ...
        m = result['metrics']
        dlg = tk.Toplevel(self)
        dlg.title(f"回測報告 — {s.get('name')} ({s.get('symbol')} {s.get('timeframe')})")
        dlg.configure(bg="#1A2026")
        self.center_window(dlg, 1040, 680)
        dlg.transient(self)
        try:
            dlg.lift(); dlg.focus_force()
        except Exception:
        ...
        cells = [
            ("總損益", f"{m['total_pnl']:+,.2f}", '#FF1744' if m['total_pnl'] > 0 else ('#00E676' if m['total_pnl'] < 0 else 'white')),
            ...
            ("獲利因子", pf, '#FFCA28'),
            ("最大回撤", f"{m['max_drawdown']:,.2f}", '#00E676'),
            ("平均持有", f"{m['avg_bars_held']:.1f} 根", 'white'),
        ...
        cells2 = [
            ...
            ("期望值/筆", f"{m.get('expectancy', 0):+,.0f}", 'white'),
            ("賺賠比", wl_txt, '#FFCA28'),
            ("最大連勝/連敗", f"{m.get('max_consec_wins', 0)}/{m.get('max_consec_losses', 0)}", 'white'),
            ("總成本(費用)", f"{m.get('total_fee', 0):,.0f}", 'white'),
            ("買進持有對照", f"{bh:+,.0f} ({m.get('buy_hold_pct', 0):+.1f}%)",
        ...
        for i, t in enumerate(result['trades'], 1):
            ...
            tvt.insert("", tk.END, values=(
                ...
                _fmt_ts(t['exit_ts']), f"{t['exit_price']:g}", f"{t['pnl']:+,.2f}",
    ...
    def _qt_open_paper_window(self):
        ...
        cells = [("初始資金", f"{a['initial_cash']:,.0f}", 'white'),
                 ("現金", f"{a['cash']:,.0f}", 'white'),
                 ("權益數", f"{eq:,.0f}", '#FFCA28'),
                 ("已實現損益", f"{a['realized_pnl']:+,.0f}", '#FF1744' if a['realized_pnl'] > 0 else ('#00E676' if a['realized_pnl'] < 0 else 'white')),
                 ("未實現損益", f"{unreal:+,.0f}", '#FF1744' if unreal > 0 else ('#00E676' if unreal < 0 else 'white')),
                 ("報酬率", f"{ret_pct:+.2f}%", '#FF1744' if ret_pct > 0 else ('#00E676' if ret_pct < 0 else 'white'))]
        for i, (lab, val, col) in enumerate(cells):
        ...
        for sym, p in a['positions'].items():
            ...
            tvp.insert("", tk.END, values=(sym, p['market'], p['direction'], p['qty'],
        ...
        for rec in reversed(a['history'][-200:]):
            ...
            tvh.insert("", tk.END, values=(rec['ts'], rec['symbol'], rec['action'], kind_txt,
                                            rec['qty'], f"{rec['price']:g}", f"{rec['fee']:,.0f}",
                                            f"{rec['pnl']:+,.0f}" if rec['kind'] == 'CLOSE' else '--'), tags=(tag,))
        foot = tk.Frame(dlg, bg="#1A2026"); foot.pack(pady=8)
        def _reset():
            try:
            ...
            self._qt_save_paper()
            self.log_message(f"【模擬帳戶】已重置,初始資金 {cash:,.0f}。")
