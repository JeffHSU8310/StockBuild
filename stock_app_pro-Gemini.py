...
class StockTradingAppPro(tk.Tk):
    def __init__(self):
        ...
        # 【效能優化】UI 更新髒標記 (Dirty Flags)
        # 用於將高頻 WebSocket 封包接收與低頻 UI 繪製徹底解耦，大幅降低延遲與 CPU 消耗
        self._quote_dirty_normal = False
        self._quote_dirty_odd = False
        self._feed_dirty_normal = False
        self._feed_dirty_odd = False
        ...
        self.log_message("【系統啟初】已阻斷初始 YF 加載。請先完成券商實盤 API 驗證登入。")
    ...
    def _record_trade_tick(self, tick_close, tick_vol, is_odd):
        ...
        # 【效能優化】取消此處直接排程更新 UI，改設髒標記交由 worker 節流處理
        try:
            ...
            with self.quote_lock:
                ...
                if is_odd:
                    self._feed_dirty_odd = True
                else:
                    self._feed_dirty_normal = True
        except Exception: pass
            ...
            if self.current_contract and tick.code == self.current_contract.code:
                ...
                with self.quote_lock:
                    if is_odd:
                        ...
                        self._quote_dirty_odd = True  # 【效能優化】標記髒資料
                    else:
                        self.current_tick_normal = tick
                        self._quote_dirty_normal = True  # 【效能優化】標記髒資料
            ...
            if self.current_contract and bidask.code == self.current_contract.code:
                with self.quote_lock:
                    if bool(getattr(bidask, 'intraday_odd', False)):
                        ...
                        self._quote_dirty_odd = True
                    else:
                        self.current_bidask_normal = bidask
                        self._quote_dirty_normal = True
        except Exception: pass
    ...
    def fetch_realtime_worker(self):
        """
        【效能優化核心】
        將原本的 0.5 秒直接迴圈縮減為 0.05 秒 (20 FPS)，配合髒標記(Dirty Flag)
        將高頻寫入與 UI 更新隔離，從根本上解決看盤軟體「反應慢半拍」的問題。
        """
        while True:
            if self._closing:
            ...
            if self.api_logged_in and HAS_SJ and self.current_symbol:
                try:
                    ...
                    # 1. 在極短的時間內鎖定記憶體，僅獲取需要更新的指標與數據
                    with self.quote_lock:
                        contract = self.current_contract
                        # 取得當前模式的最新資料，供備援檢查使用
                        curr_bidask = self.current_bidask_odd if self.is_odd_lot else self.current_bidask_normal
                        curr_tick = self.current_tick_odd if self.is_odd_lot else self.current_tick_normal

                        if self.is_odd_lot:
                            if self._quote_dirty_odd:
                                bidask = self.current_bidask_odd
                                tick = self.current_tick_odd
                                update_quote = True
                                self._quote_dirty_odd = False
                            if self._feed_dirty_odd:
                                update_feed = True
                                self._feed_dirty_odd = False
                        else:
                            if self._quote_dirty_normal:
                                bidask = self.current_bidask_normal
                                tick = self.current_tick_normal
                                update_quote = True
                                self._quote_dirty_normal = False
                            if self._feed_dirty_normal:
                                update_feed = True
                                self._feed_dirty_normal = False
                    
                    # 2. 串流更新：有新資料才排入 UI 更新，避免無意義的重繪卡死畫面
                    if update_quote:
                        self.safe_after(0, self.update_quote_ui, bidask, tick, self.is_odd_lot)
                    
                    # 3. 成交明細更新：有新跳動才更新
                    if update_feed:
                        self.safe_after(0, self._refresh_trade_feed_ui, self.is_odd_lot)

                    # 4. 無串流備援：若當下完全沒有資料 (盤後或剛換股)，每 5 秒打一次快照
                    if contract and not curr_bidask and not curr_tick:
                        now_time = time.time()
            ...
            # 【效能優化】將原本的 0.5 秒 (2 FPS) 縮短為 0.05 秒 (20 FPS)，達到肉眼無延遲
            time.sleep(0.05)
    ...
    def draw_chart(self, raw_df):
        try:
            ...
            # chart_frame (這正是我們要的「填滿可用空間」)，真正控制圖表四周
            # 留白的是上面那幾條「邊界」滑桿 (subplots_adjust)，所以這兩條多餘且會
            # 誤導,直接移除。
